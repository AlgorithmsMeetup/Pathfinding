...
describe('Pathfinding Algorithms', function(){
  ...
  xcontext('A* between two points', function(){
    it('returns the shortest path between two nodes', function(){
      var graph = createCoordinatesGraph();
...
describe('Node', function(){
  ...
  it('can have coordinates', function(){
    var node = new Node('1',2, {x:3, y:4});
    expect(node.coordinates).to.deep.equal({x:3, y:4});
  });
});
...
function createTrickyGraph(){
  var start = new Node('start', 1, {x:1, y:1});
  var node2 = new Node('2',     2, {x:2, y:2});
  var node3 = new Node('3',     1, {x:3, y:4});
  var node4 = new Node('4',     1, {x:1, y:9});
  var node5 = new Node('5',     3, {x:6, y:8});
  var end = new Node('end',     1, {x:9, y:9});
  start.connectTo(node2);
  node2.connectTo(node4);
